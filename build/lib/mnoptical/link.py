...
class Link(object):
    ...
    def propagate(self, is_last_port=False, safe_switch=False):
        ...
        # for optical_signal in self.optical_signals:
        #     print("[link 1] componet %s propagate: index %s, power: %edB, ase_noise: %edB, nli_noise: %e dB, osnr: %e, gosnr: %e" % (first_component.__class__.__name__, optical_signal.index, optical_signal.power , optical_signal.ase_noise, optical_signal.nli_noise,optical_signal.get_osnr(), optical_signal.get_gosnr()))
        ...
        # for optical_signal in self.optical_signals:
        #     print("[link 2] componet %s  propagate:index %s, power: %edB, ase_noise: %edB, nli_noise: %e dB, osnr: %e, gosnr: %e" % (first_component.__class__.__name__, optical_signal.index, optical_signal.power , optical_signal.ase_noise, optical_signal.nli_noise,optical_signal.get_osnr(), optical_signal.get_gosnr()))
...
class Span(object):
    ...
    def propagate(self, optical_signals=None, is_last_port=False, safe_switch=False):
        ...
        if hasattr(component, 'switch'):
            # print("[span] next component switch", component.__class__.__name__)
            if is_last_port:
                component.switch(in_port, self.link.src_node, safe_switch=safe_switch)
        elif hasattr(component, 'propagate'):
            # print("[span] next component propagate", component.__class__.__name__)
            component.propagate(
