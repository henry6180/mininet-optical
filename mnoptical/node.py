...
from mnoptical.terminal_params import rx_thresholds, bps, sr
            ...
            if optical_signal.index in self.rx_to_channel[in_port]['channel_id']:
                ...
                if gosnr < rx_transceiver.rx_threshold_dB:
                    ...
                    if self.debugger:
                        ...
                        print("*** power: %fdB ase_noise: %f nli_noise: %f ***" % (optical_signal.power, optical_signal.ase_noise, optical_signal.nli_noise))
...
class OpticalSignal(object):
    ...
    def get_osnr(self):
        ...
        """
        if self.ase_noise != 0:
            osnr_linear = self.power / self.ase_noise
    ...
    def get_gosnr(self):
        ...
        :return: gOSNR (linear)
        """
        if self.ase_noise != 0 or self.nli_noise != 0:
            gosnr_linear = self.power / (self.ase_noise + self.nli_noise)
...
class Amplifier(Node):
    ...
    def propagate(self, optical_signals, is_last_port=False, safe_switch=False):
        ...
        if component:
            # print("amp next component", component.__class__.__name__)
...
class Monitor(Node):
    ...
    def get_ber(self, ber_method=None):
        ...
        ber = ber_method
        if ber_method == 'bpsk':
            ber = 0.5 * erfc(sqrt(gosnr[0][1]))
        if ber_method == 'qpsk':
            ber = 0.5 * erfc(sqrt(gosnr[0][1] / 2))
        if ber_method == '8psk':
